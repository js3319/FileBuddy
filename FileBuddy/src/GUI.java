...
public class GUI {
	...
	private void initialize() {
		...
		frame.setBounds(50, 50, dm.width, dm.height);
		...
		tabbedPane.setBounds(0, 0, dm.width, dm.height);
		...
		btnNewButton.setBounds((dm.width / 2) - 475, (dm.height / 2) - 150, 130, 43);
		...
		btnStart.setBounds(358, 234, 130, 43);
		...
		openFile.setBounds(btnStart.getX() + 150, btnStart.getY(), 130, 43);
		...
		btnNewButton_4.setBounds(openFile.getX() + 150, openFile.getY(), 130, 43);
		...
		btnNewButton_3.setBounds(808, 234, 130, 43);
		...
		btnNewButton_2.setBounds(btnNewButton_3.getX() + 150, btnNewButton_3.getY(), 130, 43);
		...
		lblFileOrganizer.setBounds((int) ((dm.width / 2) - 129.5), (dm.height / 2) - 265, 259, 57);
		...
		btnNewButton_5.setBounds(btnStart.getX(), btnStart.getY() + 100, 130, 43);
		...
		btnNewButton_7.setBounds(btnStart.getX() + btnStart.getWidth() + 97, 334, 130, 43);
		...
		btnNewButton_6.setBounds(btnNewButton_3.getX(), btnNewButton.getY() + 100, 130, 43);
		btnNewButton_6.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(null,
						"<html>1. If this is your first time using this program, you need to set a password. If you wish to remove your password click the Remove Password button. All passwords are encrypted.<br> 2. You need to select a File as a reference point.<br>3. Choose operation to initiate, Organize organizes the file you selected by file extension type, Graph shows a visual of your disk space of the selected file's parent directory.</html>");
...
//					}
	...
	public void getDirectory(File fileName) {
		...
		for (File file : fList) {
			...
			if (file.isFile()) {
				...
				// filesSize.add(file.getPath()+",
				// "+df.format(file.length()/1000000000.0)+"GB");
	...
	public void checkDupe() {
		...
		for (int x = 0; x < fileContainer.size(); x++) {
			for (int y = 0; y < fileContainer.size(); y++) {
				if (fileContainer.get(x) != null && fileContainer.get(y) != null
						&& fileContainer.get(x).compareTo(fileContainer.get(y)) == 0 && (x != y)) {
	...
	public void createChart() {
		chart = new PieChartBuilder().width((int) dm.getWidth() - 300).height((int) dm.getHeight() - 300)
				.title(fileName.getParent()).theme(ChartTheme.GGPlot2).build();
		...
		for (int x = 0; x < fileContainer.size(); x++) {
			...
			if (fileContainer.get(x).fileExtension() != "" && fileContainer.get(x) != null) {
				...
				chart.addSeries(fileContainer.get(x).fileExtension() + fixedName(fileContainer.get(x).getSize()),
						fileContainer.get(x).getSize());
			}
